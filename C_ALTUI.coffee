...
altui_Svs = 'urn:upnp-org:serviceId:altui1'
...
saveVar = (deviceID, service, varname, varVal, reload) ->
	set_device_state deviceID, altui_Svs, varName, varVal, 1 if reload else 0

#-------------------------------------------------------------
# Helper functions to build URLs to call VERA code from JS
#-------------------------------------------------------------
buildVeraURL = (deviceId, fnToUse, varName, varValue) ->
	urlHead = "#{ip_address}id=lu_action&serviceId=urn:micasaverde-com:serviceId:HomeAutomationGateway1&action=RunLua&Code="
	if varValue != null
		'#{urlHead}#{fnToUse}("#{altui_Svs}", "#{varName}", "#{varValue}", #{deviceID})'
	else
		'#{urlHead}#{fnToUse}("#{altui_Svs}", "#{varName}", #{deviceID})'

buildVariableSetUrl = (deviceID, varName, varValue) ->
	"#{ip_address}id=variableset&DeviceNum=#{deviceID}&serviceId=#{altui_Svs}&Variable=#{varName}&Value=#{varValue}"

buildUPnpActionUrl = (deviceID, service, action, params) ->
	urlHead = "#{ip_address}id=action&output_format=json&DeviceNum=#{deviceID}&serviceId=#{service}&action=#{action}"
	if params?
