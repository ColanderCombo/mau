...
class UPnPHelper
    ...
    proxify: (url) -> 
        if @_ipaddr then @_proxyhead + encodeURIComponent(url) else url
    ...
    unproxifyResult: (data, textStatus, jqXHR, cbfunc) ->
        ...
        cbData = data
        if @_ipaddr
            cbData = if data[0] == "1" then data.substr(2) else null
    ...
    _exec: (url, cbfunc, mimetype) ->
        ...
        # return $.ajax(options).done (data, textStatus, jqXHR) ->
        #     @unproxifyResult(data,textStatus,jqXHR, (data,textStatus,jqXHR) ->
        ...
        return $.ajax({
                ...
                @_uproxifyResult data, textStatus, jqXHR, (data, textStatus, jqXHR) ->
                    if $.isFunction cbfunc
                        ...
                        cbfunc(if (result != null and result.length >= 2) then result[1] else null)
            .fail (jqXHR, textStatus, errorThrown) ->
                if $.isFunction cbfunc
                    cbfunc(null)
