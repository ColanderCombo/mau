...
class HTMLUtils
    @array2Table: (arr,idcolumn,viscols) ->
        ...
        if arr and $.isArray arr and arr.length > 0
            ...
            html += "<thead><tr>"
            for k,v of arr[0]
                dataId = if k == idcolumn then "data-identifier='true'" else ""
...
"""
        ...
        <div class='panel panel-default' id='#{panel.id}'>
            ...
            <div id='collapse#{panel.id}' class='panel-collapse collapse #{ if bFirst then 'in' else ''}'>
                <div class='panel-body'>
                    #{if panel.html then panel.html else _T('Empty')}
                </div>
            </div>
        </div>
"""
...
class MyLocalStorage
    ...
    @get: (key) ->
        ...
        return if json? then JSON.parse(json) else null
        ...
        if MyLocalStorage.getSettings('UseVeraFavorites')
            switch type
                when 'device'
                    ...
                    MultiBox.setAttr(device, 'onDashboard', if bFavorite then 1 else 0)
                when 'scene'
                    scene = MultiBox.getSceneByAltuiID(id)
                    scene.onDashboard = if bFavorite then  1 else 0
